...
export async function POST(req: Request) {
  try {
    const { formId, to, subject, fields } = await req.json();
...
const transporter = nodemailer.createTransport({
  ...
  tls: {
    ...
    const toAddress = to || "salam@entrepreneursmorocco.com";
    const template = FORM_TEMPLATES[formId];
    const mailSubject = subject || `${template?.title || 'Nouveau formulaire'} - entrepreneursmorocco.com`;
