...
export default function DevisComponent() {
  ...
  const getServicesLabels = (keys: string[] | undefined) => {
    ...
    return keys
      .map((k) => SERVICES_OPTS.find(([o]) => o === k)?.[1] || k)
      .join(", ");
  };
        ...
        {showSuccess ? (
          <div className="max-w-lg mx-auto text-center animate-[fadeUp_0.5s_ease-out_both]">
            <div className="v3-glass p-12" style={{ borderRadius: 28 }}>
              ...
              <h2 className="font-[family-name:var(--font-montserrat)] font-black text-2xl text-white uppercase">
                Demande reçue !
              </h2>
              <p className="text-[var(--text-secondary)] text-sm mt-4">
                Notre équipe analyse votre projet. Offre personnalisée sous 48h.
              </p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="text-center mb-14 animate-[fadeUp_0.8s_ease-out_both]">
              <div className="flex items-center justify-center gap-3 mb-6">
                ...
                <span className="v3-section-eyebrow-text">
                  Devis sur-mesure
                </span>
                <div className="para-bars para-bars--sm">
              ...
              <p className="text-[var(--text-muted)] text-sm mt-4 max-w-lg mx-auto">
                Parlez-nous de vos besoins et nous vous répondrons avec une
                offre adaptée sous 48h.
              </p>
            </div>
            ...
            <div className="max-w-2xl mx-auto animate-[fadeUp_0.8s_ease-out_0.2s_both]">
              ...
              >
                ...
                >
                  <div>
                    <label className="text-[10px] font-semibold tracking-[0.15em] uppercase text-[var(--text-muted)] mb-2 block">
                      Prénom & Nom *
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    >
                      ...
                      <ChevronDown
                        size={16}
                        className={`text-[var(--text-muted)] transition-transform duration-300 ${servicesOpen ? "rotate-180" : ""}`}
                      />
                    </button>

                    <div
                      className={`grid grid-cols-2 gap-3 overflow-hidden transition-all duration-300 ${servicesOpen ? "mt-4 max-h-[300px] opacity-100" : "max-h-0 opacity-0"}`}
                    >
                      {SERVICES_OPTS.map(([key, label]) => {
                        ...
                        return (
                          <label
                            key={key}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all duration-300 border ${sel ? "bg-[var(--accent)]/10 border-[var(--accent)]/30 text-white" : "bg-white/[0.03] border-white/[0.06] text-[var(--text-secondary)] hover:border-white/[0.12]"}`}
                          >
                            <div
                              className={`w-4 h-4 rounded flex-shrink-0 border flex items-center justify-center ${sel ? "bg-[var(--accent)] border-[var(--accent)]" : "border-white/[0.15]"}`}
                            >
                              {sel && (
                                <svg
